class liferay {
  ...
  exec {"get-liferay":
    command => "wget http://downloads.sourceforge.net/project/lportal/Liferay%20Portal/6.2.0%20GA1/liferay-portal-tomcat-6.2.0-ce-ga1-20131101192857659.zip -O /home/vagrant/liferay.zip",
    cwd => "/home/vagrant",
    path => ["/usr/bin", "/bin"],
  ...
  file {"portal-ext.properties":
    path => "${install_path}/liferay-portal-6.2.0-ce-ga1/tomcat-7.0.42/webapps/ROOT/WEB-INF/classes/portal-ext.properties",
    source => "puppet:///modules/liferay/portal-ext.properties",
    require => Exec["unzip-liferay"]
  }

  exec {"start-liferay":
    command => "${install_path}/liferay-portal-6.2.0-ce-ga1/tomcat-7.0.42/bin/catalina.sh start",
    onlyif => "test -f ${install_path}/liferay-portal-6.2.0-ce-ga1/tomcat-7.0.42/bin/catalina.sh",
    path => ["/usr/bin", "/bin"],
